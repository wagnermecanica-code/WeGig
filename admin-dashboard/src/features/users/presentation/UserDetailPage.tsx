...
import { ArrowLeft, Ban, FileText, MessageSquare, ShieldAlert } from 'lucide-react';
...
export function UserDetailPage() {
  ...
  if (!profile) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/users" className="text-primary hover:underline text-sm mt-2 inline-block">
          ← Voltar para lista
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <Link to="/users" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-primary">
        ...
        <CardBody className="flex flex-col sm:flex-row gap-4 sm:items-center">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-lg font-semibold dark:text-white">{profile.name}</h2>
              {profile.banned ? <Badge tone="danger">Banido</Badge> : <Badge tone="success">Ativo</Badge>}
              {profile.profileType ? <Badge tone="info">{profile.profileType}</Badge> : null}
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard label="Posts" value={activity?.postsCount ?? '—'} icon={<FileText className="h-4 w-4" />} />
        <StatCard
          label="Conversas"
